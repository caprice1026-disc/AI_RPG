...
`principal_id` は内部の安定IDで、認証adapterが `(issuer, subject)` から解決する。`auth_context` は `mfa` 等の認証強度を表す不透明な値であり、Campaign roleやresource権限をtoken claimからコピーしない。Applicationは `principal_id` を使って最新のCampaign membership、actor操作権、item所有権をrepository/policyで毎回認可する。
...
HTTP層の `Authenticator` portがcredentialを検証し、このvalue objectを生成する。OIDC、署名JWT、server sessionへ交換しても `(issuer, subject)` の一意対応、内部 `principal_id`、RequestContext契約を維持する。認可は独立した `AuthorizationPolicy` portに保つ。

## 影響

ログには `principal_id` とcorrelation IDを記録できるがtoken、cookie、認証秘密は記録しない。日時はtimezone-aware UTCとする。issuer/subjectの紐付け変更には監査可能なidentity migrationが必要になる。
