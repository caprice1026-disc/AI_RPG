...
narration_statusはpending → generating → completedまたはfallback。描写完了時は短いトランザクションで文章、Choice、GMNarrationGeneratedを保存する。多重workerによる上書きを防ぐため、条件付きUPDATEとworker所有トークンを用いる。DDLのworker_epoch／lease_untilは解決用であり、描写の本格worker実装時には別の所有トークンを追加するか、行ロックとジョブ一意キーで同等に制御する。
