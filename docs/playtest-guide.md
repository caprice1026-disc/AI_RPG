...
管理者はゲームのURLと、登録済みのログインアカウントを参加者へ渡します。プレイヤーはUUID、APIキー、DB接続情報を入力しません。初回は「ログインして冒険を始める」から進み、シナリオ、冒険者のタイプ、名前、能力ポイント2点の配分、得意技能を選びます。

行動候補は、サーバーに登録された行動をそのまま実行します。自由入力では別の方法や道筋も試せます。重大な結末リスクは事前に確認でき、取り消しても行動は進みません。判定や敵の反撃でHPが減るため、所持品と撤退の候補も確認してください。失敗後の方針転換・別の解決・撤退・敗北も短編の結果として扱います。
...
最初の試遊は追加説明なしで進め、迷った箇所をメモします。途中でブラウザを一度閉じて再開し、いずれかの結末まで到達してください。20〜30分は目安であり、早期達成も正当な攻略です。
